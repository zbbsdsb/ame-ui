...
export const ModelRouterDashboard = () => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-black overflow-hidden">
        ...
        <div className="w-[400px] p-4 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar space-y-2">
            ...
            {inferenceHistory.length === 0 && (
              <div className="h-full flex items-center justify-center font-mono text-[9px] text-slate-700 uppercase">
                Waiting for trace...
              </div>
            )}
          </div>
      ...
      <div className="p-3 bg-slate-900/20 flex items-center gap-6">
        ...
        <div className="ml-auto flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-2 py-1 border border-ame-border bg-slate-950 font-mono text-[10px]">
...
const ModelCard = ({ model, onTrigger }: { model: ModelEntry, onTrigger: () => void }) => {
  ...
  return (
    <button 
      onClick={onTrigger}
      className="flex items-center justify-between p-2 bg-slate-900/50 border border-ame-border hover:border-ame-accent transition-colors group text-left"
    >
      <div className="flex items-center gap-3">
        <div className="p-1.5 bg-black border border-ame-border group-hover:border-ame-accent transition-all">
          {tierIcon}
        </div>
        <div>
          <div className="font-mono text-[11px] text-white leading-none mb-1">{model.name}</div>
          <div className="flex items-center gap-2">
            <span className="font-mono text-[8px] text-slate-500">[{model.tier}]</span>
            <span className="font-mono text-[8px] text-slate-500">{model.latency}ms</span>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-1 px-1.5 py-0.5 border border-ame-border bg-black">
        <div className="w-1 h-1 rounded-full bg-emerald-500 animate-pulse" />
        <span className="font-mono text-[8px] text-slate-400">ONLINE</span>
      </div>
    </button>
  );
};
...
const HistoryItem = ({ log }: { log: InferenceLog }) => (
  <motion.div 
    ...
    className="p-2 border-l-2 border-ame-accent bg-slate-950 border border-ame-border hover:border-slate-800 transition-colors"
  >
    <div className="flex justify-between items-start mb-1">
      <span className="font-mono text-[10px] text-white uppercase tracking-tighter">{log.modelId}</span>
      <span className="font-mono text-[8px] text-slate-600">{log.timestamp}</span>
    </div>
