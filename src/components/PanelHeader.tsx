...
export const PanelHeader = ({ title, icon: Icon, extra }: PanelHeaderProps) => (
  <div className="h-9 border-b border-ame-border flex items-center px-3 justify-between bg-black/40 backdrop-blur-md relative overflow-hidden group">
    ...
    <div className="flex items-center gap-2.5 relative z-10">
      <div className="p-1 bg-slate-900 border border-slate-800">
        <Icon className="w-3 h-3 text-ame-accent opacity-70" />
      </div>
      <div className="flex flex-col">
        <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-300 leading-none">{title}</span>
        <div className="h-[1px] w-4 bg-ame-accent/30 mt-1" />
      </div>
    </div>
    <div className="relative z-10">
      {extra || <ChevronDown className="w-3 h-3 text-slate-700" />}
    </div>
  </div>
);
